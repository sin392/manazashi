...
if __name__ == "__main__":
    ...
    import cv2
    img = cv2.imread("/home/shin/Manazashi/sample.png")
    img_pil = Image.fromarray(img)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    net = MTCNN(image_size=512, thresholds=[0.5, 0.6, 0.6], margin=0, keep_all=True, device=device)
    rects, probs = net.detect(img_pil)
    print(rects.shape)
    for rect in rects:
        rect = list(map(int, rect))
        cv2.rectangle(img, tuple(rect[:2]), tuple(rect[2:]), (255, 255, 0))
    # cv2.imshow("img", img)
    # cv2.waitKey(0)
    cv2.imwrite("mtcnn.png", img)