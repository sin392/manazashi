...
def main():
    ...
    parser.add_argument('-o', '--out', help='Directory to output the result')
    ...
    for video in glob.glob(os.path.join(args.input, "*.mp4")):

        folder = os.path.splitext(video)[0]
        os.makedirs(os.path.join(args.out, folder), exist_ok=True)
        
        sub_folder = os.path.join(args.out, folder, f'Image_r{rate}')
